...
L1, L2 = 20.0, 15.0         # longitudes de los eslabones
xc, yc = -8.81, 3.22         # base del robot
...
let last_angles = nothing  # para continuidad
    anim = @animate for k in 1:length(traj)
        ...
        plot!(xtrebolscaled, ytrebolscaled; label="Círculo trébol (escalado)", color=:green, linestyle=:dashdot, linewidth=1)
        plot!(xmax, ymax; label="Límite máximo", color=:gray70, linestyle=:dashdot, linewidth=1)
        ...
        plot!([B[1], E[1]], [B[2], E[2]]; label="Eslabón 1 (20)", color=:dodgerblue, linewidth=4)
        plot!([E[1], P[1]], [E[2], P[2]]; label="Eslabón 2 (15)", color=:orange, linewidth=4)
